...
void real_game(int num_player, int initial_num, int n, int num_round, FILE* fp){
    ...
    for (int j = 0; j < num_round; ++j) {//begin the rounds
        ...
        if (j == 0) {
            turn = rwho_first(deck, number, discard, disnum, players, num_player, fp);
            number -= num_player;
            disnum += num_player;
        } else {
            turn = whowin;
